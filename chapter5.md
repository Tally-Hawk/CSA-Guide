...
# **جلسات ویندوز (Windows Sessions)**

## **جلسه تعاملی (Interactive)**

جلسه تعاملی یا ورود محلی زمانی آغاز می‌شود که کاربر با وارد کردن نام کاربری و رمز عبور خود به سیستم محلی یا دامنه احراز هویت کند.  
ورود تعاملی می‌تواند به روش‌های زیر انجام شود:

- ورود مستقیم به سیستم (Local Console)
- استفاده از دستور `runas` در خط فرمان برای ایجاد جلسه ثانویه
- اتصال از طریق **Remote Desktop** (RDP)

## **جلسه غیرتعاملی (Non-interactive)**

حساب‌های غیرتعاملی در ویندوز برخلاف حساب‌های کاربری معمولی نیازی به وارد کردن اعتبارنامه ندارند.  
سه نوع حساب غیرتعاملی اصلی وجود دارد:

- حساب سیستم محلی (Local System Account)
- حساب سرویس محلی (Local Service Account)
- حساب سرویس شبکه (Network Service Account)

این حساب‌ها معمولاً توسط سیستم‌عامل ویندوز برای راه‌اندازی خودکار سرویس‌ها و برنامه‌ها بدون نیاز به دخالت کاربر استفاده می‌شوند.  
این حساب‌ها هیچ رمز عبوری ندارند و اغلب هنگام بوت شدن سیستم یا اجرای وظایف زمان‌بندی‌شده به کار گرفته می‌شوند.

### **تفاوت‌های سه حساب غیرتعاملی**

| **حساب**                  | **توضیحات** |
|---------------------------|--------------------------------------------------------------------------------|
| **حساب سیستم محلی**<br>(NT AUTHORITY\SYSTEM) | قدرتمندترین حساب در ویندوز است. حتی از اعضای گروه Administrators محلی نیز دسترسی بیشتری دارد. برای انجام وظایف حیاتی سیستم‌عامل مانند راه‌اندازی سرویس‌های ویندوز استفاده می‌شود. |
| **حساب سرویس محلی**<br>(NT AUTHORITY\LocalService) | نسخه محدودتر حساب SYSTEM است. دسترسی‌های مشابه یک کاربر محلی معمولی دارد و فقط می‌تواند برخی سرویس‌ها را اجرا کند. |
| **حساب سرویس شبکه**<br>(NT AUTHORITY\NetworkService) | شبیه به یک حساب کاربری عادی دامنه است. روی سیستم محلی دسترسی‌های مشابه LocalService دارد، اما می‌تواند برای برخی سرویس‌های شبکه جلسه احراز هویت‌شده برقرار کند. |

# **تعامل با سیستم‌عامل ویندوز**  

### **مقدمه**
رابط کاربری گرافیکی (GUI) ویندوز امکان تعامل ساده با سیستم را فراهم می‌کند، اما برای انجام کارهای پیشرفته، اتوماسیون و مدیریت سرور، استفاده از خط فرمان (Command Line) بسیار قدرتمندتر و سریع‌تر است. در این سند با ابزارهای اصلی خط فرمان ویندوز آشنا می‌شویم.

### **پروتکل دسکتاپ از راه دور (Remote Desktop Protocol - RDP)**
- امکان اتصال کامل به دسکتاپ یک سیستم ویندوزی دیگر از راه دور  
- مناسب برای مدیریت سرورها و کمک فنی  
- ابزار پیش‌فرض ویندوز: **mstsc.exe**  
- برای اتصال سریع کافی است در Run یا CMD تایپ کنید:  
  `mstsc /v:آدرس-IP-یا-نام-کامپیوتر`

### **خط فرمان ویندوز (Windows Command Line)**

#### **CMD (Command Prompt)**
ابزار قدیمی و همچنان پرکاربرد ویندوز  
برای باز کردن:  
`Win + R` → تایپ `cmd` → Enter

**پرکاربردترین دستورات CMD:**
- `dir` → نمایش لیست فایل‌ها و پوشه‌ها  
- `cd` → تغییر دایرکتوری  
- `mkdir` → ساخت پوشه جدید  
- `copy` / `xcopy` → کپی فایل و پوشه  
- `del` → حذف فایل  
- `ipconfig` → نمایش اطلاعات شبکه  
- `ping` → تست ارتباط شبکه  
- `tasklist` → نمایش فرآیندهای در حال اجرا  
- `taskkill` → پایان دادن اجباری به یک فرآیند  
- `sfc /scannow` → بررسی و تعمیر فایل‌های سیستمی  
- `chkdsk` → بررسی و تعمیر دیسک  

#### **PowerShell**
جایگزین مدرن و بسیار قدرتمند CMD  
دارای قابلیت اسکریپت‌نویسی پیشرفته و پشتیبانی از Objectها

برای باز کردن:  
`Win + X` → Windows PowerShell یا Windows Terminal

**نمونه دستورات مفید PowerShell:**
- `Get-Process` → نمایش فرآیندها  
- `Get-Service` → نمایش وضعیت سرویس‌ها  
- `Get-ChildItem` → معادل پیشرفته dir  
- `Restart-Computer` → ری‌استارت سیستم  
- `Test-Connection` → معادل ping ولی قوی‌تر  
- `Get-Alias` → نمایش نام مستعار دستورات  

### **مستعارها (Aliases) در PowerShell**
- `dir` → `Get-ChildItem`  
- `ls` → `Get-ChildItem`  
- `cd` → `Set-Location`  
- `copy` → `Copy-Item`  
...
### **اجرای اسکریپت‌ها در PowerShell**
برای اجرای اسکریپت‌های .ps1 باید سیاست اجرا (Execution Policy) را تغییر دهید:

```powershell
# نمایش سیاست فعلی
Get-ExecutionPolicy

# تغییر به حالت مجاز (برای جلسه فعلی)
...
- **Restricted** → هیچ اسکریپتی اجرا نمی‌شود (پیش‌فرض)  
- **RemoteSigned** → اسکریپت‌های محلی اجرا می‌شوند، اسکریپت‌های دانلودشده نیاز به امضا دارند  
- **Unrestricted** → همه اسکریپت‌ها اجرا می‌شوند (خطرناک)  
- **Bypass** → هیچ محدودیتی ندارد (برای اتوماسیون)

### **نکات مهم امنیتی
- هرگز بدون دلیل از `Unrestricted` یا `Bypass` استفاده نکنید  
- اسکریپت‌های دانلودشده از اینترنت را قبل از اجرا بررسی کنید  
- برای کارهای روزمره، `RemoteSigned` بهترین تعادل بین امنیت و راحتی است

### **ابزارهای جانبی پیشنهادی
- **Windows Terminal** → جایگزین زیبا و قدرتمند برای CMD و PowerShell  
- **ConEmu** یا **Cmder** → ترمینال‌های پیشرفته با قابلیت تب و سفارشی‌سازی  
- **Chocolatey** → مدیر بسته برای ویندوز (مانند apt در لینوکس)  
- **Scoop** → مدیر بسته سبک و مدرن

**نتیجه‌گیری:**  
یادگیری خط فرمان ویندوز (به‌ویژه PowerShell) یکی از مهارت‌های ضروری برای مدیران سیستم، برنامه‌نویسان و کاربران حرفه‌ای است. با تمرین مداوم این ابزارها، سرعت و دقت کارتان با ویندوز چندین برابر خواهد شد.



# **ابزار مدیریت ویندوز (WMI)**

**ابزار مدیریت ویندوز (WMI)** زیرسیستمی از پاورشل است که به مدیران سیستم ابزارهای قدرتمندی برای نظارت بر سیستم ارائه می‌دهد. هدف اصلی این ابزار، یکپارچه‌سازی مدیریت دستگاه‌ها و برنامه‌ها در شبکه‌های سازمانی است. این ابزار از ویندوز ۲۰۰۰ به بعد به‌صورت پیش‌فرض در سیستم‌عامل ویندوز نصب شده و بخش اصلی آن محسوب می‌شود. این ابزار از اجزای زیر تشکیل شده است:

### **اجزای اصلی WMI**

| **جزء**              | **نام**                  | **توضیحات**                                                                                  |
|-----------------------|--------------------------|-----------------------------------------------------------------------------------------------|
|                      | سرویس WMI               | فرآیند ابزار مدیریت ویندوز که به‌صورت خودکار هنگام بوت اجرا می‌شود و نقش واسطه بین مشتریان WMI و ارائه‌دهندگان آن را ایفا می‌کند تا داده‌ها را بازیابی کند. |
|                      | اشیاء مدیریت‌شده        | هر جزء منطقی یا فیزیکی که قابل مدیریت توسط WMI باشد.                                         |
|                      | ارائه‌دهندگان WMI       | اشیایی که رویدادها و داده‌های مربوط به یک شیء خاص را نظارت می‌کنند.                           |
|                      | کلاس‌ها                  | توسط ارائه‌دهندگان WMI برای انتقال داده به سرویس WMI استفاده می‌شوند.                        |
|                      | متدها                   | به کلاس‌ها متصل هستند و امکان انجام عملیات را فراهم می‌کنند. مثلاً می‌توان از متدها برای شروع یا توقف فرآیندها در ماشین‌های راه‌دور استفاده کرد. |
|                      | مخزن WMI                 | پایگاه داده‌ای که تمام داده‌های ثابت مربوط به WMI را ذخیره می‌کند.                            |
|                      | مدیر اشیاء CIM          | سیستمی که داده‌ها را از ارائه‌دهندگان WMI درخواست کرده و به برنامه درخواست‌کننده بازمی‌گرداند. |
|                      | رابط برنامه‌نویسی WMI    | امکان دسترسی برنامه‌ها به زیرساخت WMI را فراهم می‌کند.                                        |
|                      | مصرف‌کننده WMI           | پرس‌وجوها را از طریق مدیر اشیاء CIM به اشیاء ارسال می‌کند.                                  |

### **کاربردهای WMI**

- دریافت اطلاعات وضعیت سیستم‌های محلی یا راه‌دور
- پیکربندی تنظیمات امنیتی روی ماشین‌ها یا برنامه‌های راه‌دور
- تنظیم و تغییر مجوزهای کاربران و گروه‌ها
- تنظیم یا تغییر ویژگی‌های سیستم
- اجرای کد
- زمان‌بندی فرآیندها
- راه‌اندازی ثبت لاگ‌ها

این وظایف را می‌توان با ترکیب **پاورشل** و رابط خط فرمان WMI (**WMIC**) انجام داد.

**WMIC** منسوخ شده است. برای اجرای آن در خط فرمان ویندوز، کافی است دستور `WMIC` را تایپ کنید تا پوسته تعاملی باز شود یا مستقیماً دستوری مانند `WMIC computersystem get name` را اجرا کنید تا نام میزبان دریافت شود. برای مشاهده لیست دستورات و aliasها، از `WMIC /?` استفاده کنید.

### **سوئیچ‌های جهانی WMIC**

- **/NAMESPACE**: مسیر فضای نام که aliasها روی آن عمل می‌کنند.
- **/NODE**: سرورهایی که alias روی آنها اجرا می‌شود.
- **/USER** و **/PASSWORD**: کاربر و رمز عبور برای ورود به جلسه.
- **/PRIVILEGES**: فعال یا غیرفعال کردن تمام privilegesها.
- **/OUTPUT**: ارسال خروجی به فایل یا چاپگر.
- و سایر سوئیچ‌ها...

### **مثال عملی با WMIC**

برای نمایش اطلاعات مختصر سیستم‌عامل:
...
|-------------|--------------|----------------|-------------------|--------------------|------------|
| 15204       |              | domain\user    | 123456789-00123-ABCD | C:\Windows\system32 | 10.0.19041 |

### **استفاده از WMI در پاورشل**

در پاورشل از cmdletهای زیر استفاده می‌شود:

- **Get-WmiObject**: برای دریافت نمونه‌های کلاس‌های WMI یا اطلاعات درباره کلاس‌های موجود (قابل استفاده روی ماشین محلی یا راه‌دور).
...
- **Invoke-WmiMethod**: برای فراخوانی متدهای اشیاء WMI.

مثال تغییر نام فایل:

```powershell
Invoke-WmiMethod -Path "CIM_DataFile.Name='C:\users\public\apps.csv'" -Name Rename -ArgumentList "newname.csv"
```

اگر `ReturnValue` برابر ۰ باشد، عملیات با موفقیت انجام شده است.

### **نکته پایانی**

این بخش مروری کوتاه بر **WMI**، **WMIC** و ترکیب آنها با پاورشل ارائه داد. این ابزار کاربردهای گسترده‌ای برای تیم‌های آبی (دفاعی) و قرمز (تهاجمی) دارد. در بخش‌های بعدی دوره، نحوه استفاده تهاجمی از WMIC برای شناسایی (enumeration) و حرکت جانبی (lateral movement) بررسی خواهد شد.
