...
- **Shebang (`#!/bin/bash` یا `#!/usr/bin/env bash`)**  
  خط اول هر اسکریپت که به سیستم‌عامل می‌گوید از چه مفسر (interpreter) برای اجرای اسکریپت استفاده کند. استفاده از `#!/usr/bin/env bash` قابل‌حمل‌تر است چون مسیر دقیق بش را در سیستم‌های مختلف پیدا می‌کند.
